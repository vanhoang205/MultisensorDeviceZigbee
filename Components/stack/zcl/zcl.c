...
{
  zclConfigReportRecsList *pNewItem;    // this pointer will hold config. report recs list to added
  zclConfigReportRecsList *pLoop;       // this pointer is used to point to end of list
  
  // Allocate memory heap for config. report recs list variable
  pNewItem = zcl_mem_alloc( sizeof( zclConfigReportRecsList ));
  if ( pNewItem == NULL )       // when memory is full, it notice failure initialization
  {
    return (ZMemError);
  }
  
  // Assigns value to this pointer
  ...
  // If list is empty, this list will add the first item pNewItem
  if ( configReportRecsList == NULL )
  {
    configReportRecsList = pNewItem;
  }
  else  // if this list have any item "config. report rec list" , it will find the lastest location and point to new item will be added
  {
    // Look for end of list
                              ...
                              zclConfigReportRec_t **pConfigReportRec )
{
  ...
  if ( pRec != NULL )
  {
    ...
    {
      if (pRec->configReportRecs[x].clusterId == clusterId &&
          pRec->configReportRecs[x].cfgReportRec.attrID == attrId )
      {
